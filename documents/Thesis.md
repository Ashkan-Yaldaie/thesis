...
For security reasons,  the internet cable remained disconnected from the router before changing the password, it is connected again after rebooting the device. `sudo reboot` is the terminal command for restarting the RasPi. Next the system is updated by entering these commands in the following order:
