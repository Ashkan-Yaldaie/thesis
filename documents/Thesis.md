...
After installing the software and its dependencies, the RF transmitter and the receiver are connected to the RasPi as shown in Figure 8 with their data pins attached to the GPIO 0 and the GPIO 2. In order to see the GPIO table and pin numbering details, the terminal command `gpio readall` is used. The receiver is just needed to record the RF codes transmitted by the sockets remote control and it can be disconnected from the RasPi once the data is captured.
